...
def analysePerformance(aggregator, nb_clients, nb_byz, res, senario):
    ...
    log.writeLogs()
    res[aggregator]['time'] = times
    res[aggregator]['end_time'] = end_times
...
def violin_plot(fl, p2p, con):
    ...
    ax = sns.violinplot(data = df,x = 'aggregator', y = 'time', hue='senario', fill=False)
...
def all_accuracy_per_round(fl, p2p, con):
    for agg in aggregators:
        for att in attacks:
            ...
            plt.savefig("../../Plots/" + agg + "/" +  att + "/" + str(nb_clients) + "_"  + str(nb_byz) + "_rounds.png", bbox_inches='tight')
            plt.close()
...
def all_accuracy_per_time(fl, p2p, con):
    for agg in aggregators:
        for att in attacks:
            ...
            ax.errorbar(x=[np.mean(t) for t in p2p[agg]['end_time'].values()], y=[np.mean(t) for t in con[agg][att][1:]], yerr=[np.std(t) for t in con[agg][att][1:]], xerr=[np.std(t) for t in con[agg]['end_time'].values()], fmt='o')

            ax.set_xlabel('time(s)')
            ax.set_ylabel('accuracy')
            ax.legend(['fl', 'p2p', 'con'])
            ax.set_title("average accuracy per average time for n = " + str(nb_clients) + ", n byz = " + str(nb_byz) + "\naggregator: " + agg + "\nattack:" + att)
            plt.savefig("../../Plots/" + agg + "/" +  att + "/" + str(nb_clients) + "_"  + str(nb_byz) + "_average_time.png", bbox_inches='tight')
            plt.close()
            ...
            ax.errorbar(x=[np.min(t) for t in p2p[agg]['end_time'].values()], y=[np.mean(t) for t in con[agg][att][1:]], yerr=[np.std(t) for t in con[agg][att][1:]], fmt='o')
            ...
            ax.errorbar(x=[np.max(t) for t in p2p[agg]['end_time'].values()], y=[np.mean(t) for t in con[agg][att][1:]], yerr=[np.std(t) for t in con[agg][att][1:]], fmt='o')
